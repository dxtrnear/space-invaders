...
class Game(object):#cette classe permet de gerer les elements du jeu
    ...
    def affiche_name(self):
        self.score_actuel = StringVar()  #score actuel
        score_label = Label(self.canvas, textvariable = self.score_actuel, bg="black", fg="white", font="Arial 13")
        self.score_actuel.set("Welcome : " + str(self.score.get_name())) #initialisation du score
        self.canvas.create_window(100, 25, window = score_label)
    ...
    def lost(self):#affiche un ecran de fin en supprimant tout les autres elements
        ...
        self.canvas.create_text(x,y,text="GAME OVER", fill="red", font="Helvetica 30 bold")
    def win(self):#affiche un ecran de fin en supprimant tout les autres elements
        x,y = int(self.canvas.cget("width"))//2 , int(self.canvas.cget("height"))//2
        self.canvas.delete("all")
        self.score.game_state=True
        self.canvas.create_text(x,y,text="You won !", fill="green", font="Helvetica 30 bold")
...
class SpaceInvaders(object):
    ...
    def close(self):
        self.root.quit()
    def menu(self):
        self.game.canvas.create_text(self.game.width/2,120,text="Space invader", fill="lime", font="Helvetica 50 bold")
        self.player_name=Entry(self.game.canvas, text="Enter your name",bg="gray", bd=1)
    ...
    def start(self,event):
        self.game.score.name=self.player_name.get()
        self.game.canvas.delete("all")
        self.player_name.destroy()
        self.game.canvas.grid_forget()
